...
function statusColor(s: string) {
  const m: Record<string, string> = {
    ...
    CONCLUIDA: 'var(--success)', CANCELADA: 'var(--muted)', EXPIRADA: 'var(--danger)',
  };
...
export default function HistoricoVeiculo() {
  ...
  const totalGasto = resultados
    .filter(r => r.status?.toUpperCase() === 'CONCLUIDA')
    .length; // placeholder — valor real viria de /api/pagamentos cruzado
    ...
    <section>
      <div className="page-title card">
        <div>
          ...
          <p>Consulte todo o histórico de um veículo ou cliente pelo nome.</p>
        </div>
      </div>
          ...
          {resultados.length > 0 && (
            <div style={{ display: 'flex', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
              {[
                ...
                { label: 'Última visita', value: resultados[0] ? dataHora(resultados[0].criadoEm).split(' ')[0] : '—' },
                ...
                <table>
                  ...
                  <tbody>
                    {resultados.map(r => (
                      <tr key={r.id}>
                        <td>
                          <strong style={{ fontFamily: 'monospace', color: 'var(--text)', letterSpacing: '0.05em' }}>
                            {r.placa || '—'}
                          </strong>
                          {r.modeloVeiculo && (
                            <div style={{ fontSize: 11, color: 'var(--muted)' }}>{r.modeloVeiculo}</div>
                          )}
                        </td>
                        ...
                        <td style={{ fontSize: 12 }}>
                          {tempoTotal(r.horarioChegadaReal || r.horarioChegadaPrevisto, r.horarioSaidaReal || r.horarioSaidaPrevisto)}
                        </td>
                        <td>
                          <span style={{
